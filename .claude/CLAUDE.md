...
Páginas construidas: `/` (landing), `/registro` y `/login` (**ambos conectados a Supabase Auth real** — signUp/signInWithPassword), `/perfil` (**conectado de verdad** — lee y guarda en la tabla `profiles` de Supabase según persona/empresa), `/terminos`, `/privacidad` (borradores legales), `/admin` (**protegido con autenticación real** vía `AdminGate.tsx`, solo cuentas con `app_metadata.role = "admin"`; el guardado de los ajustes del back office sigue siendo vista previa). `Header.tsx` es client component: muestra el `primer_nombre` (persona) o `razon_social` (empresa) leyendo la tabla `profiles`, con el correo como respaldo, y botón "Cerrar sesión" cuando hay sesión.

Backend: **Supabase confirmado y conectado** (Auth + Postgres). Proyecto `colombia-contrata`, ref `zjbijmieiyumpqwyqhfm`, región São Paulo. Cliente en `src/lib/supabase.ts`, usa `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY`. Tabla `profiles` (RLS habilitado, columnas separadas nulas por tipo de cuenta) documentada con el SQL completo en el README → "Base de datos (Postgres)". Storage aún no se usa.
...
- **Auth + DB**: **Supabase confirmado y conectado** (proyecto `colombia-contrata`, ref `zjbijmieiyumpqwyqhfm`, región São Paulo). Auth + tabla `profiles` en Postgres ya en uso. Falta storage con URLs firmadas para la expiración de 10 días de los documentos.
