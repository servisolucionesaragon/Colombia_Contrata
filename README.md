...
src/
  ...
  components/
    ...
    InstallPwaBanner.tsx    # barra que recomienda instalar la PWA en móvil/tablet (agregada en Header.tsx, debajo de la franja de colores); botón "Instalar" en Android, instrucciones manuales en iOS Safari
...
✅ **Validado de punta a punta contra Wompi real (2026-09-04)**: en cuanto el usuario activó su cuenta de Wompi y configuró llaves reales de Sandbox desde `/admin` → Pagos (Wompi), se hizo una compra de prueba real en `/solicitar` con la tarjeta de pruebas de Wompi (`4242 4242 4242 4242`) — el pago se aprobó en `checkout.wompi.co`, confirmando por primera vez que la firma de integridad es correcta. El webhook, en cambio, no actualizaba `solicitudes.estado` — el "Debugger" de Wompi (registro de eventos enviados) mostraba **cero eventos**, ni siquiera fallidos. Causa raíz (encontrada con `curl -D -`): la **URL de Eventos** de Wompi (configurada manualmente en `comercios.wompi.co` → Desarrollo → Programadores, no en el código) apuntaba a `https://colombiacontrata.com/api/webhooks/wompi` — el dominio raíz, que Vercel redirige (`308 Permanent Redirect`) hacia `https://www.colombiacontrata.com/...`. Wompi no sigue ese redirect al enviar el evento (a diferencia de un navegador, que sí lo sigue sin problema — por eso `redirect-url`/`siteUrl()` nunca tuvo este problema), así que el webhook nunca llegaba al código real. Se corrigió la URL de Eventos de Sandbox a la versión con `www.`, y una tercera compra de prueba sí actualizó `solicitudes.estado` a `"pagado"` — **firma de integridad y checksum del webhook, ambos validados contra Wompi real por primera vez**. Las filas de prueba generadas se borraron por SQL después. ⚠️ **Pendiente**: al configurar la URL de Eventos de Producción en el dashboard de Wompi, usar también la versión con `www.` desde el principio.

✅ **Generación real de documentos para personas, conectada (2026-09-05)**: el `TODO` del webhook (arriba) ya está resuelto. `src/lib/solicitudVerificacion.ts` (mismo patrón que `consultaDecision.ts` del flujo de empresa) se encarga de: leer el perfil de la persona (`profiles`) y las fuentes seleccionadas en su checklist al pagar (`solicitudes.documentos[].clave_fuente`), llamar a `consultarVerificacionCompleta` **solo con esas fuentes** (no las 26), subir los PDF resultantes al mismo bucket privado `verificaciones-pdf` que usa el flujo de empresa (`{solicitudId}/{fuente}.pdf`, sin colisión posible con las carpetas `{consultaId}/...` porque son UUID de tablas distintas), y guardar el resultado en las mismas columnas `resultado_*`/`nivel_riesgo` que ya existían en `consultas` (agregadas ahora también a `solicitudes`). El webhook de Wompi (`/api/webhooks/wompi`) dispara esto con `after()` en cuanto una solicitud pasa a `"pagado"`, con `resultado_obtenido_at` como guarda de idempotencia (un reintento de Wompi del mismo evento no vuelve a gastar el crédito de Vericol). `/historial` muestra una columna "Resultado" con el mismo botón "Ver documentos" que ya usaba `/empresas/consultas` — se extrajo a `DocumentosBoton.tsx` (antes duplicado dentro de `EmpresaConsultasContent.tsx`) y `DocumentosResultado.tsx` pasó a aceptar `tipo: "consultas" | "solicitudes"` para armar la URL correcta (`/api/consultas/[id]/...` o `/api/solicitudes/[id]/...`); esta última ruta nueva usa `src/lib/solicitudAcceso.ts`, que autoriza **solo al propio dueño de la solicitud** (a diferencia de `consultaAcceso.ts`, donde el candidato nunca ve sus documentos — aquí la persona es la única dueña legítima, no hay "empresa" de por medio). `FUENTE_LABEL` (nombre bonito de cada fuente, antes solo 6 claves hardcodeadas dentro de `DocumentosResultado.tsx`) se centralizó en `solverio.ts` cubriendo las 26. Verificado end-to-end en producción con una compra real en Sandbox seleccionando solo 3 documentos (ADRES, Contraloría, Registraduría): el webhook disparó la verificación automáticamente, `/historial` mostró "Ver documentos (3)" — ni más ni menos — con los 3 PDF reales descargables (confirmado leyendo la URL firmada real generada y el tamaño de cada archivo en Storage), riesgo clasificado, y "Descargar todo (.zip)" funcionando. Datos de prueba (fila de `solicitudes` y los 3 PDF en Storage) borrados después.

⚠️ **Costo real, sin ambiente de pruebas para Vericol**: a diferencia de Wompi (que sí tiene Sandbox sin dinero real), Vericol no tiene ningún modo de pruebas en esta integración — **cualquier solicitud que llegue a `"pagado"` gasta un crédito real** de la cuenta de Vericol del usuario, sin importar si el pago en Wompi fue en Sandbox o Producción. Tenerlo presente antes de hacer más pruebas de pago en `/solicitar`.

✅ **Correo de "documentos listos" + fuentes sin PDF visibles en pantalla (2026-09-05)**: el usuario reportó dos cosas después de una prueba real: (1) la página de confirmación del pago promete "te notificaremos por correo cuando tus documentos estén listos", pero ese correo nunca se construyó; (2) al consultar 17 fuentes solo aparecían 9 (las que traen PDF) — las 8 restantes sí devuelven un resultado real de Vericol (`estado`, a veces con `error`), pero quedaban invisibles porque `DocumentosResultado.tsx` solo renderizaba lo que había en `resultado_pdfs`. Ambos se corrigieron: `procesarPagoAprobadoSolicitud` (en `solicitudVerificacion.ts`) manda un correo real vía Resend (`plantillaDocumentosListos`, mismo lenguaje visual que `plantillaInvitacionConsulta`) apenas termina la verificación con éxito — busca el correo del usuario con `db.auth.admin.getUserById` (no hay columna de email en `solicitudes`/`profiles`); si falla el envío, es best-effort y no afecta el resultado ya guardado. `DocumentosResultado.tsx` ahora también lee `resultado_json.data.fuentes` (el mismo array que ya usa `ConsultaManualAdmin.tsx` en `/admin`, con forma `{fuente, estado, error, tieneSoporte}`) y muestra una sección "Otros resultados (sin documento descargable)" para las fuentes que no tienen PDF — el `estado`/`error` se muestra tal cual lo devuelve el proveedor, sin traducir códigos que no están documentados. El conteo del botón "Ver resultados (N)" (antes "Ver documentos (N)") ahora cuenta el total de fuentes con resultado, no solo las que tienen PDF. Aplica a ambos flujos (empresa y persona) ya que comparten el mismo componente. Verificado en producción con la consulta real de 17 fuentes del usuario: la sección nueva mostró las 8 fuentes faltantes (SECOP, INPEC, PEPs, Función Pública, RUT (DIAN), Sisbén IV/RUI —con un mensaje de error real de la fuente—, Licencia de conducción, RUES), sumando 17 en total. **El envío del correo no se ha verificado con una entrega real todavía** — solo se dispara en una verificación nueva (no en datos ya guardados), así que falta confirmar en la próxima solicitud pagada real.
...
⚠️ **Bug real encontrado y corregido — el periodo mensual/anual no se recordaba al pasar del landing a `/empresas/planes`** (2026-09-05): el usuario reportó "cuando selecciono pago anual, me muestra el valor mensual y ese es el mismo valor que me lleva a Wompi". Causa: `PlanesEmpresaPricing.tsx` (el landing público en `/`) muestra el precio anual dividido en 12 cuando el visitante activa el toggle "Anual" (patrón estándar de pricing — "$X/mes, facturado anualmente", con el total real como texto secundario abajo), pero su botón "Elegir plan" enlazaba siempre a `/empresas/planes` sin decirle qué periodo se había elegido — y `EmpresaPlanesContent.tsx` (la página que sí cobra) siempre abría con el toggle en `"mensual"` por defecto. Si el usuario compraba ahí sin volver a marcar "Anual" a mano, se cobraba el valor mensual completo pensando que era el anual. Se corrigió pasando el periodo por query string (`href={\`/empresas/planes?periodo=${periodo}\`}`) y haciendo que `EmpresaPlanesContent.tsx` lo lea en el `useState` inicial (`new URLSearchParams(window.location.search).get("periodo")`) — verificado en producción que el enlace ya trae `?periodo=anual` y que `URLSearchParams` lo parsea bien.

**Hallazgo colateral (dato, no bug de código)**: el plan **"Expert"** tiene `precio_anual = $9.450.000` en la base de datos, **idéntico al de "Advanced"** — casi seguro un error de captura (copy-paste), no un descuento real. El patrón de descuento del resto de los planes es progresivo con el tamaño del plan (Lite/Essential 15%, Standard 18%, Advanced 21%, Professional 31%, Business 40%); interpolando ese patrón, a Expert (entre Advanced y Professional) le correspondería un anual de **~$25.000.000** (27% de descuento). No se corrigió en código porque el precio es una decisión de negocio del usuario — queda pendiente que él lo ajuste desde `/admin` → Planes de empresa.
...
- El usuario verificó el sitio en Google Search Console y envió el sitemap ahí mismo el 2026-08-17 (paso que solo él podía hacer, con su propia cuenta de Google) — con esto el sitio quedó indexable de punta a punta, no solo técnicamente listo.
...
**Ícono PWA se corrigió dos veces el mismo día, a pedido del usuario**:
1. El usuario probó desde su celular y reportó que el ícono se veía muy pequeño. Causa: `apple-touch-icon.png` reutilizaba el mismo margen de seguridad del 62% pensado para el ícono "maskable" de Android, pero iOS solo redondea las esquinas del ícono en vez de recortarlo a formas distintas, así que no necesitaba tanto margen — se subió a 86% de relleno (y el maskable de Android de 62% a 70%, dentro de la zona segura recomendada pero menos perdido dentro del cuadro).
2. El usuario pidió, además, que el ícono PWA use la misma imagen configurable en `/admin` → Identidad del portal → Ícono/Favicon (`configuracion_portal.favicon_url`), en vez de un archivo fijo generado del isotipo — así que si cambia el favicon, el ícono de "instalar app" cambia con él. `manifest.ts` y `layout.tsx` (`icons.apple`) ahora leen `favicon_url` con fallback a los archivos generados solo si nunca se configuró uno. ⚠️ **Gotcha**: al agregarlo, `manifest.ts` seguía sirviendo un valor viejo pese al cambio en la base de datos — faltaba `export const revalidate = 60`, sin eso Next.js trata el manifest como estático y lo genera una sola vez en build time (mismo patrón ya usado en `layout.tsx`/`page.tsx` por la misma razón). Ya corregido.

**Banner de recomendación de instalación** (mismo pedido del usuario, "cuando abran el portal desde dispositivos móviles o tablets, recomiende instalar la aplicación"): `InstallPwaBanner.tsx` (nuevo, agregado en `Header.tsx` justo después de la franja de colores de marca, visible en todo el sitio) — detecta la plataforma por `navigator.userAgent`:
- **Android/Chrome**: escucha el evento `beforeinstallprompt`, lo guarda, y muestra la barra con un botón "Instalar" que llama a `deferredPrompt.prompt()` cuando el usuario lo toca.
- **iOS Safari** (no tiene ese evento — Apple no permite disparar el diálogo de instalación por JS): muestra instrucciones manuales ("Compartir" → "Agregar a inicio") directamente, sin esperar ningún evento. Se excluyen a propósito Chrome/Firefox/Edge en iOS (`CriOS`/`FxiOS`/`EdgiOS` en el user agent) porque en iOS solo Safari puede instalarse como app independiente. iPadOS 13+ se identifica como "Macintosh" en el user agent — se detecta por `navigator.maxTouchPoints > 1` además del user agent.
- No aparece si el sitio ya corre en modo `standalone` (ya instalado), en desktop, ni después de cerrarlo una vez (se guarda en `localStorage`).
...
## Cambio de proveedor a Vericol + catálogo de documentos con fuentes reales (2026-09-05)

El proveedor (mismo servicio de arriba, ahora identificado en su propia colección de Postman como **"Vericol API"**) reemplazó el endpoint Enterprise de solo-lectura fija por uno que exige elegir explícitamente qué fuentes consultar:

- **`POST /api/Verificacion/completa`** (reemplaza al `GET /api/enterprise/verificacion/completa` de arriba) — el body incluye `fuentes: string[]` con las claves a consultar y `obtenerSoportes: true` para pedir los PDF. El costo en créditos depende de cuántas fuentes se pidan: ≤15 = 1 crédito, 16–30 = 2, más de 30 = 3 (+1 crédito extra si alguna fuente entrega PDF). `src/lib/solverio.ts` → `consultarVerificacionCompleta(config, datos, fuentes)` ahora recibe ese arreglo como tercer parámetro, con `TODAS_LAS_FUENTES` (las 26 claves reales, exportada) como valor por defecto cuando no hay checklist que filtrar.
- **Catálogo `precios_documentos` rehecho como espejo 1:1 de las 26 fuentes** — se agregaron las columnas `descripcion` (texto explicativo) y `clave_fuente` (una de las 26 claves de Vericol, o `null` si el documento es solo informativo sin fuente automática equivalente). Cada fila del checklist ahora corresponde exactamente a una fuente real: `ADRES`, `BDME`, `Contraloría`, `Función Pública`, `INPEC`, `Delitos contra menores`, `Listas restrictivas`, `PACO`, `PEPs`, `Policía Nacional`, `Procuraduría`, `Proveedores ficticios`, `Licencia de conducción`, `RNMC`, `RUCOM`, `RUES`, `RUT (DIAN)`, `Rama Judicial`, `Registraduría`, `SECOP`, `SENA`, `SIMIT`, `SIRI`, `Sisbén IV / RUI`, `SISCONMP`, `Supersociedades`. `FUENTE_LABEL` (en `solverio.ts`) centraliza el nombre bonito de cada clave para mostrarlo en botones/nombres de archivo PDF.
- **El checklist ya filtra de verdad qué se consulta** — antes era solo informativo (siempre se pedía el conjunto fijo completo). Ahora, al invitar un candidato o solicitar como persona, `documentos_requeridos`/`documentos` (el snapshot guardado en `consultas`/`solicitudes` al momento de la acción) incluye la `clave_fuente` de cada documento elegido, y `ejecutarVerificacion`/`procesarPagoAprobadoSolicitud` arman el arreglo `fuentes` a partir de esas claves — solo se pide (y se cobra) lo que el usuario seleccionó. Verificado en producción seleccionando deliberadamente 3 de 26 documentos: la respuesta y el resultado mostrado en pantalla trajeron exactamente esas 3 fuentes.
- **Generación real de documentos para personas** (antes solo existía para el flujo de empresa/candidato) — `src/lib/solicitudVerificacion.ts` (mismo patrón que `consultaDecision.ts`) se dispara desde `/api/webhooks/wompi` cuando una `solicitud` de `/solicitar` pasa a `"pagado"`: lee el perfil de la persona + su checklist guardado, llama a Vericol solo con esas fuentes, sube los PDF al mismo bucket `verificaciones-pdf`, y guarda el resultado en las mismas columnas `resultado_*`/`nivel_riesgo` que ya tenía `consultas` (agregadas ahora también a `solicitudes`). `resultado_obtenido_at` es la guarda de idempotencia si Wompi reenvía el mismo evento. `src/lib/solicitudAcceso.ts` autoriza la descarga **solo al dueño de la solicitud** (a diferencia de `consultaAcceso.ts`, donde el candidato nunca ve sus propios documentos — aquí no hay "empresa" de por medio). `/api/solicitudes/[id]/pdf` y `/pdf-zip` son los endpoints equivalentes a los de `consultas`. Un correo real (`plantillaDocumentosListos`, vía Resend) avisa a la persona cuando la verificación termina con éxito, cumpliendo la promesa que ya hacía la pantalla de confirmación del pago.
- **`DocumentosResultado.tsx` generalizado**: acepta `id` + `tipo: "consultas" | "solicitudes"` en vez de un `consultaId` fijo, y `DocumentosBoton.tsx` (extraído, antes vivía duplicado dentro de `EmpresaConsultasContent.tsx`) se reusa en ambos flujos. No todas las fuentes devuelven PDF — las que solo traen un resultado en JSON (`resultado_json.data.fuentes`, con forma `{fuente, estado, error, tieneSoporte}`, el mismo shape que ya leía `ConsultaManualAdmin.tsx`) ahora también se muestran, en una sección "Otros resultados (sin documento descargable)", en vez de quedar invisibles. El botón pasó de "Ver documentos (N)" a "Ver resultados (N)", contando el total real de fuentes con resultado.
- **Ícono "genera PDF" en los checklists**: nueva columna `precios_documentos.genera_pdf` (booleano), marcada `true` en las 9 fuentes que se confirmaron con PDF en una consulta real (`adres`, `contraloria`, `delitosMenores`, `policia`, `procuraduria`, `ramaJudicial`, `registraduria`, `rnmc`, `simit`) — basado en observación real, no en documentación oficial del proveedor, así que es editable desde `/admin` → Documentos disponibles si Vericol cambia de comportamiento para alguna fuente. `GeneraPdfBadge.tsx` (compartido) muestra un pequeño ícono de documento junto al nombre en los tres checklists (persona, empresa individual, carga masiva) y en el admin, con una leyenda explicativa junto al checkbox "Todos".

✅ **Confirmado contra respuestas reales (2026-09-05)**: la estructura `{exitoso, mensaje, data}` con `data.nivelRiesgo` numérico sí aplica también a este endpoint POST — validado con varias consultas reales del usuario (una de 12 fuentes, otra de 17). De paso se descubrió el detalle completo de `data`: además de metadatos (`consultaId`, `nombreCompleto`, `nivelRiesgo`, `estadoConsulta`, `fuentesOk`, `fuentesNoDisponibles`, `alertas`, `consumo*`, `autorizacion*`) y el array `data.fuentes` (**solo el log de ejecución** — `{fuente, estado, error, duracionMs, tieneSoporte}`, donde `estado: "ok"` significa nada más que "la llamada no falló"), **`data` siempre trae una clave por cada una de las 26 fuentes posibles** (`data.adres`, `data.secop`, etc. — `null` si esa fuente no se pidió o no devolvió nada) con el **resultado sustantivo real** de cada una: mensajes ya redactados por la fuente (`data.funcionPublica.mensaje: "No se encontraron registros en SIGEP para 'Juan Pérez'."`), banderas de hallazgo (`data.siri.tieneInhabilidades`, `data.sisconmp.tieneCapacitacion`), listados completos (`data.simit.cursos`/`.multas`), e incluso, para algunas fuentes, su propio PDF embebido en base64 (`data.simit.soportePdf`) además del que ya se sube a Storage vía el mapa agregado `data.soportesPdf`.

⚠️ **El usuario reportó "en Otros resultados... debería mostrar un resultado que se envía desde la fuente" en vez de "ok"** — el bug era leer solo `data.fuentes[i].estado` (el log técnico) sin tocar `data.<claveFuente>` (el contenido real). Se corrigió en `DocumentosResultado.tsx`: `resumenDetalleFuente()` ahora usa el campo `mensaje` de la fuente cuando existe (es literalmente la frase que ya redactó el proveedor), y si no hay `mensaje` arma un resumen corto con los campos `tieneX`/`esX` (booleanos) y `totalX` (números) que sí aparecen en casi todas las fuentes. De paso se encontró un **desajuste de nombres real entre dos partes de la misma respuesta de Vericol**: el log de ejecución identifica una fuente como `"proveedoresFicticiosDian"`, pero su resultado vive en `data.proveedoresFicticios` (sin "Dian") — se agregó `ALIAS_CLAVE_DETALLE` en `DocumentosResultado.tsx` para ese caso puntual (y como lugar donde agregar más si aparecen otros). Verificado en producción con la consulta real de 12 fuentes del usuario: SIRI, Función Pública, SISCONMP y Proveedores ficticios (las 4 sin PDF de esa consulta) pasaron de mostrar "ok" a mostrar su hallazgo real.
...
**Envío del correo**: `src/lib/resend.ts` (`enviarCorreo`) llama directo a la API HTTP de Resend (`https://api.resend.com/emails`) — **distinto** del SMTP que ya usa Supabase Auth para sus propios correos (confirmación, cambio de contraseña), que no sirve para mandar correos con contenido propio. Llaves en `configuracion_resend` (fila única, mismo patrón write-only/RLS-sin-policies que `configuracion_wompi`/`configuracion_solverio`: solo la Service Role Key la lee).

**Plantilla con el mismo estilo que los correos de Supabase Auth**: el usuario pidió que este correo (enviado por Resend) se viera igual a los que ya envía Supabase Auth ("Confirma tu cuenta en Colombia Contrata", "contraseña fue modificada", etc.), en vez de un diseño nuevo. Se extrajo el HTML real de la plantilla "Confirm sign up" desde Supabase (Authentication → Emails → Templates) y se adaptó `plantillaInvitacionConsulta` (`src/lib/emailPlantillas.ts`) al mismo lenguaje visual: tabla de layout con barra superior azul `#0033A0`, logo (misma URL de Storage `.../Recursos/logo.png` que ya usan las plantillas de Auth), franja tricolor amarillo/azul/rojo, tarjeta blanca redondeada, y footer navy `#0D1B3D` con el mismo texto de cierre. La diferencia es la sección central: en vez de un solo botón, dos botones lado a lado (Autorizar en azul `#0033A0`, Rechazar en rojo `#CE1126`), más una nota aclarando que ningún botón ejecuta la acción por sí solo. HTML con estilos inline y tablas (necesario para que se vea bien en clientes de correo, mismo patrón que la plantilla original).
...
## Interfaz de documentos + descarga en ZIP (2026-08-17)

El usuario pidió "una interfaz amigable en la sección de documentos después de una consulta" y "un botón descargar todo, un zip que incluya todos los documentos encontrados, tanto en persona como empresa" — hasta ese momento `EmpresaConsultasContent.tsx` solo mostraba enlaces de texto sueltos por fuente, y el candidato (lado persona) no veía sus propios documentos en ningún lado tras autorizar.

**`DocumentosResultado.tsx`** (nuevo): panel con badge de nivel de riesgo + fecha de verificación, una tarjeta por documento (ícono, nombre de la fuente, ícono de descarga) y un botón "Descargar todo (.zip)". Vive en `EmpresaConsultasContent.tsx`: la columna "Documentos" de la tabla pasó de una lista de enlaces sueltos a un botón "Ver documentos (N)" que abre un modal con el panel completo.

⚠️ **Corregido el mismo día — esto es exclusivo de la empresa**: la primera versión también mostraba este panel al candidato en `/autorizaciones` (razonando que "tanto en persona como empresa" del pedido original se refería a que ambos lados vieran sus documentos). El usuario aclaró de inmediato: *"en /autorizaciones como candidato no debería ver los documentos, ya que estos documentos son solo para la empresa. Y tampoco debe saber qué documento se le está pidiendo, solo las Autorizaciones."* Se revirtió: `AutorizacionesContent.tsx` volvió a mostrar solo lo esencial de la autorización (empresa, fecha, datos del candidato, checkbox de Habeas Data, botones Autorizar/Rechazar), sin ningún documento ni lista de qué se solicita. La frase "tanto en persona como empresa" del pedido original se refería a que el botón de descarga en ZIP debía existir para los dos **flujos** de consulta que tiene el sitio (candidato de empresa vs. — en teoría — solicitud de persona), no a que el candidato viera sus propios resultados.

**Acceso restringido a la empresa** (`src/lib/consultaAcceso.ts`, `resolverAccesoDocumentos`): `/api/consultas/[id]/pdf` (URL firmada individual) y el endpoint nuevo `/pdf-zip` solo autorizan a la empresa dueña de la consulta (vía `resolverContextoEmpresa`) — el candidato nunca puede llamarlos, ni para un documento suelto ni para el zip completo.

**`GET /api/consultas/[id]/pdf-zip`** (nuevo): descarga todos los PDF de la consulta desde el bucket privado `verificaciones-pdf` (server-side, con la Service Role Key), los empaqueta en un `.zip` en memoria y lo devuelve directo como adjunto (`Content-Type: application/zip`), sin subir nada nuevo a Storage. El ZIP se arma a mano con `src/lib/zip.ts` usando el módulo nativo `zlib` (`deflateRawSync` + CRC-32 calculado con una tabla estándar) — **sin agregar ninguna librería npm** (`archiver`/`jszip`/`adm-zip`), mismo criterio de siempre en este proyecto por lo lenta que es una instalación nueva en la unidad de red. Los nombres de archivo dentro del zip se dejaron sin tildes (`Registraduria.pdf`, no `Registraduría.pdf`) a propósito — una prueba con acentos mostró que no todos los `unzip` respetan bien el flag de nombre UTF-8 del formato ZIP.

Verificado en producción con datos reales (una consulta real ya autorizada, con 5 documentos genuinos de una verificación de Solverio): el modal "Ver documentos" del lado empresa mostró el riesgo, la fecha y las 5 tarjetas correctas; `GET /pdf-zip` respondió `200` con `Content-Type: application/zip`, ~505 KB, y los primeros 4 bytes del archivo (`50 4b 03 04`) confirmaron la firma estándar de ZIP (`PK\x03\x04`) — un zip válido.

## Empresa: elegir documentos requeridos al invitar (2026-08-17)

A pedido explícito del usuario ("algo súper importante... al momento de invitar un candidato, también poder seleccionar o chequear qué documentos se requiere de la lista"): tanto en la invitación individual (`EmpresaConsultasContent.tsx`) como en la carga masiva (`CargaMasivaContent.tsx`) apareció un checklist con los mismos documentos de `precios_documentos` (pestaña "Documentos disponibles" en `/admin`) que ya se usaba en `/solicitar` para personas — la empresa marca cuáles necesita antes de poder invitar (botón deshabilitado sin al menos uno marcado). En carga masiva el checklist es **uno solo para todo el lote**, no por fila — igual que `loteReferencia`.

**Nota importante — esto es informativo, no filtra la llamada real a Solverio**: la integración actual siempre pide la "verificación completa" (10+ fuentes fijas) sin poder pedir un subconjunto — mismo comportamiento que ya tenía `/solicitar` para personas (el checklist ahí tampoco cambia qué se genera, solo el precio es fijo). Lo que sí hace este checklist es dejar constancia de qué pidió la empresa y mostrárselo al candidato **antes de que autorice** — más transparencia para el consentimiento de Habeas Data.

`consultas.documentos_requeridos jsonb` (nuevo) guarda el array `[{id, documento}]` resuelto y validado server-side en `POST /api/consultas/crear` (mismo patrón que ya usaba `/api/solicitudes/crear` con `documentoIds`) — queda como registro interno de la empresa, para su propio control de qué pidió por candidato.

⚠️ **Corregido el mismo día — el candidato no debe ver esta lista**: la primera versión mostraba "Documentos solicitados" al candidato en `/autorizaciones`, en `/consultas/responder` y en el correo de invitación. El usuario corrigió explícitamente: *"tampoco debe saber qué documento se le está pidiendo, solo las Autorizaciones."* Se quitó de los tres lugares — `/api/consultas/pendientes` y `/api/consultas/responder-token` (GET) ya ni siquiera seleccionan/devuelven la columna al candidato, y `plantillaInvitacionConsulta` volvió a su versión sin la sección de documentos. La columna sigue existiendo y sigue guardándose (la empresa la sigue eligiendo al invitar), solo dejó de exponerse al candidato en cualquier superficie.

⚠️ **Incidente real de esta sesión**: el código se subió a producción antes de confirmar que la migración (`alter table consultas add column documentos_requeridos jsonb`) ya se había aplicado — el usuario dijo "ya disponible para que continues" refiriéndose a que el Studio de Supabase ya estaba accesible para que el propio asistente corriera el SQL, no a que él ya lo había corrido. Esto se malinterpretó y se hizo `git push` con el código dependiendo de la columna nueva, **dejando roto en vivo el endpoint de invitar candidatos** (`POST /api/consultas/crear`, tanto individual como carga masiva) hasta que la migración se aplicó minutos después. Lección: cuando una migración es un prerequisito de un deploy, confirmar explícitamente "¿ya corriste el SQL?" con una respuesta inequívoca antes de hacer push — una frase ambigua como "ya está disponible" puede referirse a la herramienta, no a la acción completada.
...
## Reenviar/eliminar invitaciones, filtros, y pantallas más anchas (2026-08-18)

Mismo día, tres pedidos seguidos del usuario tras preguntarle "qué otra mejora consideras debe llevar" y mostrarle varias opciones:

**1. Botón "Reenviar invitación" y "Eliminar invitación"** — en la tabla "Consultas enviadas" de `/empresas/consultas`, visibles solo para filas en estado `pendiente` (a propósito: una consulta ya `autorizada` o `rechazada` es el registro real de lo que pasó, incluido si se descontó un crédito — no se debe poder borrar ni tiene sentido reenviar).
- `POST /api/consultas/[id]/reenviar`: genera un `token_respuesta` nuevo (invalida cualquier enlace viejo que hubiera quedado dando vueltas) y reenvía el correo real vía `enviarCorreo`/`plantillaInvitacionConsulta` — mismo mecanismo que la invitación original.
- `DELETE /api/consultas/[id]` (nuevo `route.ts` en `src/app/api/consultas/[id]/`, junto a los ya existentes `pdf/` y `pdf-zip/`): borra la fila. Ambos endpoints verifican que quien llama es la empresa dueña (`resolverContextoEmpresa`) y que el estado sigue siendo `pendiente` antes de actuar.
- Verificado por el usuario en producción: probó "Eliminar" sobre una fila de prueba real y funcionó; "Reenviar" no mandó el correo en el primer intento pero sí en el segundo (no se investigó la causa del primer fallo — podría haber sido una demora normal de Resend, no se reprodujo).

**2. Filtros y búsqueda** — `FiltrosBar.tsx` (nuevo, compartido): select de estado + rango de fechas (Desde/Hasta) + búsqueda de texto opcional, con botón "Limpiar" que aparece solo si hay algún filtro activo. Filtrado **100% client-side** sobre los datos ya cargados (sin tocar ningún endpoint ni RLS), aplicado en cinco tablas: "Consultas enviadas" de `/empresas/consultas`, y las cuatro secciones de `/historial` (Compras de créditos, Consultas enviadas, Solicitudes de documentos, Consultas de antecedentes recibidas). Verificado en producción con datos reales: filtrar por estado "Pendiente" redujo la tabla a la fila correcta, y "Limpiar" restauró las demás. El usuario pidió después que la búsqueda de texto también encontrara por **número de documento** del candidato (antes solo nombre y correo) — se agregó `candidato_numero_documento` a la concatenación buscada en las dos tablas de "Consultas enviadas" (`/empresas/consultas` e `/historial`), con el placeholder actualizado a "Nombre, correo o documento". Verificado en producción buscando por un número de documento real: filtró a la única fila correcta. El usuario también pidió mostrar la columna de documento en la tabla "Consultas enviadas" de `/historial` (ya se buscaba por ella, pero no se veía) — se agregó `candidato_tipo_documento` al `select` y una columna "Documento" (`CC 12345678`, igual formato que `/empresas/consultas`). Verificado en producción tras una recarga forzada (el navegador tenía cacheada la versión anterior de la página).
...
Verificado en producción insertando una notificación de prueba por SQL para la cuenta real de empresa (sin necesidad de autorizar/rechazar una consulta real): la campana mostró el badge "1", el dropdown mostró el mensaje y la fecha correctos, y "Marcar todas como leídas" quitó el badge, el punto azul y aclaró el texto — fila de prueba borrada después. Esa misma verificación reveló, sin haberlo buscado, que **la función ya venía funcionando de punta a punta con candidatos reales**: aparecían dos notificaciones reales previas (una autorización y un rechazo genuinos) generadas automáticamente por `notificarEmpresa()` — confirma el flujo completo sin haber tenido que gastar un crédito de Solverio a propósito para probarlo.
...
## Salida a producción: Wompi en vivo (2026-09-06)

El usuario preguntó "¿qué nos faltaría para salir a producción?" y, tras revisar el estado completo, se identificaron 4 bloqueadores reales (dinero/legal). Los tres primeros se resolvieron el mismo día:

1. **Precio anual de "Expert" corregido** — pasó de $9.450.000 (copiado por error de "Advanced") a **$25.000.000**, el valor que se había calculado interpolando el patrón de descuento de los demás planes. Verificado por SQL: el múltiplo quedó en 8.77x con 26.9% de descuento, y la progresión de descuentos del catálogo quedó perfectamente monótona (15% → 15% → 18.2% → 21.2% → **26.9%** → 31.2% → 40%). *(Nota menor: al reescribirlo, el precio mensual quedó en $2.849.898 en vez de $2.849.900 — dos pesos menos, sin efecto funcional.)*
2. **Correo de "documentos listos" confirmado** — el usuario verificó que sí llega, cerrando el pendiente que quedó abierto cuando se construyó (solo se había validado la lógica, no una entrega real).
3. **Wompi Producción ACTIVADA** — el usuario cambió `ambiente_activo` a `"produccion"` desde `/admin` → Pagos (Wompi). Verificado por SQL inmediatamente después: llave pública con prefijo `pub_prod_` (no `pub_test_`) y ambos secretos (integridad y eventos) guardados.

⚠️ **La verificación más importante de este hito**: como el bug del redirect apex→www ya había roto Sandbox una vez (ver arriba), lo primero que se revisó al activar Producción fue si la trampa seguía viva — `curl` confirmó que sí (`https://colombiacontrata.com/api/webhooks/wompi` → `308`, `https://www.colombiacontrata.com/...` → `200`). Como el usuario no estaba seguro de cómo había quedado configurada la URL de Eventos del ambiente de Producción, se le pidió revisarla en el dashboard de Wompi antes de que llegara cualquier cliente real; confirmó que quedó con `www.` incluido. **Sin esa confirmación, el primer cliente real habría pagado y sus documentos nunca se habrían generado, en silencio y sin ningún error visible.**
...
**Acceso del administrador a los documentos** (ampliado el mismo día): la primera versión mostraba solo *metadatos* (cuántos documentos se generaron, cuándo, qué falló) y dejaba los PDF accesibles únicamente por su dueño. El usuario pidió explícitamente ampliarlo — *"para soporte necesitas poder ver los documentos solicitados, abrir los PDF también desde admin, y ver las respuestas de las consultas que no generan PDF"* — porque sin eso no hay forma de responder un "pagué y el documento no abre" sin pedirle el archivo al propio titular.

El permiso se agregó en **`resolverAccesoDocumentos` (`consultaAcceso.ts`) y `resolverAccesoDocumentosSolicitud` (`solicitudAcceso.ts`)**, que son el único punto de control de los cuatro endpoints (`/api/consultas/[id]/pdf`, `/pdf-zip`, `/api/solicitudes/[id]/pdf`, `/pdf-zip`) — una sola excepción `user.app_metadata?.role === "admin"` en cada resolver, en vez de repetirla en cada ruta. **Lo que no cambió**: el candidato de una consulta sigue sin poder abrir esos documentos (ver la corrección del 2026-08-17 sobre documentos que son solo para la empresa), las URLs siguen siendo firmadas y de corta duración, y el bucket sigue privado.
...
- El correo al titular usa `plantillaRespuestaSolicitudDatos` (`emailPlantillas.ts`), **con el mismo diseño que las demás plantillas** — la primera versión era HTML plano sin maquetar. La respuesta del admin va en un bloque citado con borde azul, y el pie ofrece dos botones: escribir al correo de contacto y, si el WhatsApp está activo, abrirlo directamente. Ambos canales salen de `configuracion_portal`, no fijos en el código. ⚠️ A pedido del usuario **no se menciona la queja ante la SIC**: se prefiere ofrecer primero los canales propios, que además es el orden que pide la ley (agotar el trámite con el responsable antes de acudir a la Superintendencia).
...
## Cierre de deuda técnica antes de producción (2026-09-06)

Un bloque de pendientes que el usuario despachó de una vez, tras pedir el inventario de lo que faltaba.

- **Páginas legales fechadas** — Términos y Privacidad decían "[fecha pendiente de publicación]"; ahora dicen *1 de septiembre de 2026*. El usuario ya había completado por su cuenta la razón social y el NIT reales (SERVISOLUCIONES ARAGON, NIT 1038103291-9) en ambas.
- **Precio mensual de Expert** — pasó de `2.849.898` a `2.849.900`, cerrando los 2 pesos que quedaron de la corrección del precio anual.
- **`nombre_portal` y `eslogan` ya se leen en vivo** — se guardaban desde `/admin` → Identidad del portal desde agosto, pero **nadie los leía**. Ahora alimentan el `<title>`, la meta descripción, Open Graph, Twitter Card, el JSON-LD de organización, el manifiesto de la PWA y la marca del Header y del Footer. El bicolor de la marca se conserva **partiendo el nombre configurado** (primera palabra navy, resto azul) en vez de fijar "Colombia Contrata" en el código. Los literales quedan solo como respaldo si la fila no existe. Verificado en producción: el manifiesto ya sirve el eslogan real de la base de datos.
- **IP en la constancia de consentimiento (Habeas Data)** — pendiente desde agosto porque `supabase.auth.signUp` corre en el cliente y no conoce la IP. Se resolvió con `POST /api/consentimiento`, que lee `x-forwarded-for` y escribe en una tabla nueva `consentimientos`. **No se usa `user_metadata` a propósito**: ese campo lo puede modificar el propio usuario con `supabase.auth.updateUser`, así que no sirve como prueba; la tabla tiene RLS activo sin ninguna policy, solo la escribe el servidor. Como en el registro todavía no hay sesión (falta confirmar el correo), el endpoint no puede validar un token: en su lugar comprueba que la cuenta exista y **se acabe de crear** (menos de 5 minutos), lo que deja fuera fabricar constancias para cuentas ajenas o antiguas. Un índice único `(user_id, politica_version)` absorbe los reintentos. Es best-effort desde el formulario: si falla, la cuenta igual queda creada y el consentimiento sigue en `user_metadata`, solo sin IP.
- **Tema claro por defecto** — el script de arranque en `layout.tsx` seguía el `prefers-color-scheme` del sistema operativo del visitante, así que quien tuviera Windows/macOS en oscuro veía el sitio oscuro sin haberlo pedido. Ahora **el oscuro solo se activa si la persona lo eligió** con el interruptor del encabezado; sin preferencia guardada, siempre claro. Quien ya lo había elegido conserva su elección (queda en `localStorage`). Verificado en producción emulando un sistema en modo oscuro y sin preferencia guardada: la página carga con `background: rgb(255,255,255)` y sin la clase `.dark`; y tras pulsar el interruptor, el oscuro sobrevive a una recarga.
...
- Algunos municipios quedan con su **nombre oficial**, que puede diferir del de antes: "Cartagena de Indias", "San José de Cúcuta", "Santa Cruz de Mompox". Para no dejar en blanco a quien ya tenía guardado el nombre corto, `CitySelect` en `ProfileForm.tsx` **agrega como opción la ciudad guardada** cuando no está en la lista, **solo dentro del departamento guardado**. ⚠️ La primera versión la agregaba en cualquier departamento que se eligiera y el usuario vio "Medellín" (su ciudad guardada) en todos; se corrigió el mismo día comparando con el departamento guardado (`initialDepartamento`).
