...
⚠️ **Pendiente de confirmar**: la estructura de respuesta (`{exitoso, mensaje, data}`, `data.nivelRiesgo` numérico) se asumió igual a la del endpoint Enterprise anterior porque la colección de Postman de este nuevo endpoint tampoco trae un ejemplo — sigue sin validarse contra una respuesta real específicamente de `POST /api/Verificacion/completa` (sí se confirmó, en cambio, que trae el array `data.fuentes` con el resultado por fuente, gracias a `ConsultaManualAdmin.tsx`).
