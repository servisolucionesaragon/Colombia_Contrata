...
- **Diseño de la plantilla** (2026-08-08, a pedido del usuario "me gustaría que le pusiera un diseño más profesional conservando el contenido"): encabezado tipo hero (fondo `bg-brand-navy`, título grande en blanco, enlace "← Volver al inicio"), y el contenido HTML se envuelve en un `<div>` con selectores hijos (`[&>h2]`, `[&>h3]`, `[&>ul>li]`, etc.) que agregan divisor superior + espaciado a cada `<h2>` (separa visualmente cada sección del contenido) y viñetas con check (✓) en vez de bullets en las listas — todo por CSS, sin tocar el HTML guardado en `contenido`. Como es la plantilla compartida por todas las páginas propias, este estilo aplica automáticamente a cualquier página nueva que se cree, no solo a "Nosotros".
