...
- [ ] **Activar Producción en Wompi**: cambiar `ambiente_activo` a `"produccion"` en `/admin` → Pagos (Wompi) cuando el usuario decida empezar a cobrar de verdad (requiere confirmación explícita, ya que ahí empieza a moverse dinero real). Antes de activarlo, configurar la URL de Eventos de Producción en el dashboard de Wompi usando también la versión con `www.` (mismo gotcha que Sandbox).
