...
**`DocumentosResultado.tsx`** (nuevo, compartido): panel con badge de nivel de riesgo + fecha de verificación, una tarjeta por documento (ícono, nombre de la fuente, ícono de descarga) y un botón "Descargar todo (.zip)". Se usa en dos lugares:
- **Empresa** (`EmpresaConsultasContent.tsx`): la columna "Documentos" de la tabla pasó de una lista de enlaces a un botón "Ver documentos (N)" que abre un modal con el panel completo.
- **Persona/candidato** (`AutorizacionesContent.tsx`): se muestra inline dentro de la tarjeta de cada consulta ya autorizada — **antes el candidato no veía nada después de autorizar**, solo la empresa.

**Acceso compartido** (`src/lib/consultaAcceso.ts`, `resolverAccesoDocumentos`): antes `/api/consultas/[id]/pdf` solo dejaba descargar a la empresa dueña (vía `resolverContextoEmpresa`). Se generalizó a un helper que autoriza **tanto a la empresa como al propio candidato** (por `candidato_id` ya vinculado o por `candidato_email`, mismo criterio que ya usaba `/api/consultas/autorizar`) — lo usan tanto `/pdf` (URL firmada individual) como el endpoint nuevo `/pdf-zip`.
...
Verificado en producción con datos reales (una consulta real ya autorizada, con 5 documentos genuinos de una verificación de Solverio): el modal "Ver documentos" mostró el riesgo, la fecha y las 5 tarjetas correctas; `GET /pdf-zip` respondió `200` con `Content-Type: application/zip`, ~505 KB, y los primeros 4 bytes del archivo (`50 4b 03 04`) confirmaron la firma estándar de ZIP (`PK\x03\x04`) — un zip válido. No se probó la vista de persona/candidato de forma interactiva (no había credenciales de esa cuenta a mano en esta sesión), pero usa el mismo componente y el mismo endpoint ya verificados del lado empresa, con el mismo `resolverAccesoDocumentos` — riesgo bajo.
...
`consultas.documentos_requeridos jsonb` (nuevo) guarda el array `[{id, documento}]` resuelto y validado server-side en `POST /api/consultas/crear` (mismo patrón que ya usaba `/api/solicitudes/crear` con `documentoIds`). Se muestra al candidato en tres lugares: `/autorizaciones` (antes del checkbox de Habeas Data), `/consultas/responder` (la página pública del correo) y dentro del correo de invitación mismo (`plantillaInvitacionConsulta`, nueva sección "Documentos solicitados").
...
- [x] Interfaz amigable de documentos + descarga en `.zip` (empresa y persona) — ver [Interfaz de documentos + descarga en ZIP](#interfaz-de-documentos--descarga-en-zip-2026-08-17). Verificado en producción con una consulta real. Falta probar de forma interactiva la vista de persona/candidato (usa el mismo componente ya verificado del lado empresa).
- [x] Empresa elige documentos requeridos al invitar (individual y carga masiva) — ver [Empresa: elegir documentos requeridos al invitar](#empresa-elegir-documentos-requeridos-al-invitar-2026-08-17). Es informativo (se lo muestra al candidato antes de autorizar), no filtra la llamada a Solverio — esa integración siempre pide la verificación completa de todas formas.
