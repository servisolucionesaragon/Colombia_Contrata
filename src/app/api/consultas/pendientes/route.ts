...
export async function GET(request: NextRequest) {
  ...
  const { data: consultas } = await db
    .from("consultas")
    .select(
      "id, empresa_id, estado, created_at, candidato_primer_nombre, candidato_segundo_nombre, candidato_primer_apellido, candidato_segundo_apellido, candidato_tipo_documento, candidato_numero_documento, nivel_riesgo, resultado_pdfs, resultado_error, resultado_obtenido_at"
    )
